...
function Navigation(options) {
    ...
    if (options.showBackButton) {
        $('#nav-body-ctrls').append('<i id="nav-ctrls-back" class="nav-icons disabled" title="Nhấn để quay lại">' + this.SVG_BACK + '</i>');
    }
    if (options.showForwardButton) {
        $('#nav-body-ctrls').append('<i id="nav-ctrls-forward" class="nav-icons disabled" title="Nhấn để tiến">' + this.SVG_FORWARD + '</i>');
    }
    if (options.showReloadButton) {
        $('#nav-body-ctrls').append('<i id="nav-ctrls-reload" class="nav-icons disabled" title="Tải lại trang">' + this.SVG_RELOAD + '</i>');
    }
    if (options.showUrlBar) {
        $('#nav-body-ctrls').append('<input id="nav-ctrls-url" type="text" title="Nhập địa chỉ hoặc tìm với Google" placeholder="Nhập địa chỉ hoặc tìm kiếm với Google"/>')
    }
    if (options.showAddTabButton) {
        $('#nav-body-tabs').append('<i id="nav-tabs-add" class="nav-icons" title="Thêm tab mới">' + this.SVG_ADD + '</i>');
    }
    /**
     * ADD CORE STYLE
     */
        ...
        webview[0].addEventListener('did-fail-load', (res) => {
            if (res.validatedURL == $('#nav-ctrls-url').val() && res.errorCode != -3) {
                this.executeJavaScript('document.body.innerHTML=' +
                    ...
                    '<h2 align=center>Ôi! Trang này không thể tải được ;(</h2>' +
                    '<p align=center><i>ERROR [ ' + res.errorCode + ', ' + res.errorDescription + ' ]</i></p>' +
                    '<br/><hr/>' +
                    '<h4>Try this</h4>' +
                    '<li type=circle>Kiểm tra lỗi chính tả - <b>"' + res.validatedURL + '".</b></li><br/>' +
                    '<li type=circle><a href="javascript:location.reload();">Làm mới</a> trang này.</li><br/>' +
                    '<li type=circle>Hãy thử<a href=javascript:location.href="https://www.google.com/search?q=' + res.validatedURL + '"> tìm kiếm với Google </a>.</li><br/>' +
                    '</div>'
                );
            }
        });
...
Navigation.prototype.newTab = function (url, options) {
    ...
    // title
    if (options.title == 'default') {
        tab += '<i class="nav-tabs-title"> Đang tải... </i>';
    } else {
